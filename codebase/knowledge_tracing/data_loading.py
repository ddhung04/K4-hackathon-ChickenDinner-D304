...
import pandas as pd
from dialogue.chatlog_loader import load_vlearn_chatlog
...
def load_src_data(args, split: str = ""):
    ...
    elif args.dataset == "vlearn":
        return load_vlearn_chatlog("data/vlearn-pack/chatlog/chat_history_anonymized_for_hackathon.csv")
...
def load_annotated_data(args, fold: Union[int, str, None] = 1):
    if args.dataset == "comta":
        ...
        df = pd.read_csv(
            get_annotated_data_filename(args),
            converters={
                col: literal_eval
                for col in ["dialogue", "meta_data", "annotation"]
            },
        )
